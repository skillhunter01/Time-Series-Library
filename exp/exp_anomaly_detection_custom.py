...
# -------------- UNIVARIATE ---------------------
# class Exp_Anomaly_Detection_custom(Exp_Basic):
#     def __init__(self, args):
#         super(Exp_Anomaly_Detection_custom, self).__init__(args)
...
class Exp_Anomaly_Detection_custom(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_x_era5, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                batch_x = batch_x.float().to(self.device)
                batch_x_era5 = batch_x_era5.float().to(self.device)

                batch_x_mark = batch_x_mark.float().to(self.device)
                
                outputs = self.model(batch_x_era5, None, None, None)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            self.model.train()
            epoch_time = time.time()
            for i, (batch_x, batch_x_era5, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                iter_count += 1
                model_optim.zero_grad()
    ...
    def test(self, setting, test=0):
        ...
        # (2) find the threshold
        attens_energy = []
        for i, (batch_x, batch_x_era5 , batch_x_mark, batch_y_mark) in enumerate(test_loader):
            batch_x = batch_x.float().to(self.device)
            batch_x_era5 = batch_x_era5.float().to(self.device)
            # reconstruction
            outputs = self.model(batch_x_era5, None, None, None)
...
#                 f_dim = -1 if self.args.features == 'MS' else 0
#                 outputs = outputs[:, :, f_dim:]
#                 print(f"batch shape {batch_x.shape}, output shape {outputs.shape}")
#                 loss = criterion(outputs, batch_x)
#                 train_loss.append(loss.item())
...
#         with open('./test_results/' + setting + '/loss.txt', 'w') as f:
#             f.write("Train Loss: {0:.7f} Vali Loss: {1:.7f}".format(train_loss, vali_loss))    
