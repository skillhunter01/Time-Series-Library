...
class ANAM_era5_loader(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
    ...
    def __read_data__(self):
        ...
        if self.scale:
            ...
            train_data_era5 = df_era5[border1s[0]:border2s[0]]
            self.scaler.fit(train_data_era5.values)
            data_era5 = self.scaler.transform(df_era5.values)
        else:
            data = df_data.values
            data_era5 = df_era5.values
        ...
        self.data_x = data[border1:border2]
        self.data_era5_x = data_era5[border1:border2]
        self.data_y = data[border1:border2]
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.data_x[s_begin:s_end]
        seq_x_era5 = self.data_era5_x[s_begin:s_end]
        # seq_y = self.data_y[r_begin:r_end]
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]

        return seq_x, seq_x_era5, seq_x_mark, seq_y_mark
...
class ANAMSegloader(Dataset):
    def __init__(self, args, root_path, data_path, win_size, step=1, flag="train"):
        ...
        data = df_raw[:int(0.7 * len(df_raw))]
        data = data.values[:, 1:] 
        ...
        test_data = df_raw[int(0.8 * len(df_raw)) :]
        test_data = test_data.values[:, 1:]
        test_data = np.float32(test_data)
        test_data = np.nan_to_num(test_data)
        self.test = self.scaler.transform(test_data)

        self.val = df_raw[int(0.7 * len(df_raw)) : int(0.8 * len(df_raw))]
        self.val = self.val.values[:, 1:]
        self.val = np.float32(self.val)
